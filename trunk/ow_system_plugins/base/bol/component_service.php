...
class BOL_ComponentService
{
    ...
    public function findPlace($placeName)
    {
        ...
        $placeDto = $this->placeDtoCache[$placeName];

        if ( !$placeDto )
        {
            throw new InvalidArgumentException("Invalid place name `$placeName`");
        }
    ...
    protected function fetchSettingList( $dtoList, $componentPlaceUniqName = null )
    {
        ...
        {
            $resultList[$dto->componentPlaceUniqName][$dto->name] = $dto->value;
        }
    ...
    public function addWidgetToPlace( BOL_Component $widget, $place, $uniqName = null )
    {
        ...
        $componentPlaceDto->placeId = $placeId;

        $componentPlaceDao->save($componentPlaceDto);

        $this->componentPlaceCacheDao->deleteCache($placeId);
