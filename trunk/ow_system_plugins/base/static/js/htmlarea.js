(function($) {
    ...
    jHtmlArea.fn = jHtmlArea.prototype = {
            ...
            if (elem.nodeName.toLowerCase() === "textarea") {
                ...
                var tempStyles = function(){
                    ...
                    if( !window.htmlAreaData.tempStyles ){
                        ...
                        $.each( baseCss.cssRules,
                            function(key, data){
                                if( data.selectorText.match(/body\.htmlarea_styles/) ){
        ...
        insertImage: function(params){
            if( params.preview ){
                $html = $('<div><a href="'+params.src+'" target="_blank"><img src="'+params.src+'" /></a></div>');
            }else{
                $html = $('<div><img src="'+params.src+'" /></div>');
            }
        ...
        video: function(){
            var self = this;
            var $contents = $('<div>'+window.htmlAreaData.labels.common.videoTextareaLabel+'<br /><textarea name="code" style="height:200px;"></textarea></div>');
        ...
        insertLink: function(params){
            ...
            this.pasteHTML('<span class="ow_ws_link"><a href="'+params.url+'">'+params.label+'</a>&nbsp;</span>');
            this.tempFB.close();
        },

        link: function() {
            var self = this;
            var $contents = $('<div style="padding:10px 35px;">Text to display:<br /><input name="label" type="text" style="width:400px;" /><br /><br />To what URL should this link go:<br /><input type="text" value="http://" name="url" style="width:400px;" /><br /><br /></div>');
            var buttonCode = window.htmlAreaData.buttonCode;
    ...
    var priv = {
        toolbarButtons: {
            orderedlist: function(btn){this.orderedList(btn);},
            unorderedlist: function(btn){this.unorderedList(btn)},
            bold: function(btn){this.bold(btn);},
            italic: function(btn){this.italic(btn);},
            underline: function(btn){this.underline(btn);},
            image: function(btn){this.image(btn)},
            link: function(btn){this.link(btn)},
            switchHtml: function(btn){this.toggleHTMLView(btn);},
            video: function(btn){this.video(btn);},
            html: function(btn){this.html(btn);},
            more: function(btn){this.more(btn);}
        },
        toolbarHandlers:{
            bold: function(){return this.qcv('bold');},
            italic: function(){return this.qcv('italic');},
            underline: function(){return this.qcv('underline');},
            orderedlist: function(){return this.qcv('insertorderedlist');},
            unorderedlist: function(){return this.qcv('insertunorderedlist');}
        },
        ...
        attachEditorEvents: function() {
            ...
            var fnTA = function() {self.updateTextArea();};
            var fnTb = function(){self.updateToolbar();}
