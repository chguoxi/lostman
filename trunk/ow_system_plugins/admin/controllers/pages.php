...
class SaveForm extends Form
{
    ...
    public function __construct( OW_Renderable $rendrable )
    {
        ...
        $externalUrl->setInvitation('http://www.example.com')->setHasInvitation(true)->
            addValidator(new ExternalPageUrlValidator())->setId('url2');
    ...
    public function process( $params )
    {
        ...
        switch ( $_POST['type'] )
        {
            case 'local' :
                ...
                $document->setUri(str_replace(OW::getRouter()->getBaseUrl(), '', OW::getRouter()->getBaseUrl() . UTIL_String::removeFirstAndLastSlashes($_POST['local-url'])));
...
class LocalPageUrlValidator extends OW_Validator
{
    ...
    public function getError()
    {
        if ( $this->errCode == 2 )
        {
            return OW::getLanguage()->text('base', 'white_spaces_dissalowed');
        }
...
class ExternalPageUrlValidator extends OW_Validator
{
    ...
    public function isValid( $value )
    {
        ...
        if ( !( empty($_POST['external-url']) || empty($value) || trim($value) == '' ) )
            return true;
