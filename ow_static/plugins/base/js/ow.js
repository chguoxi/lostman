...
/*        var docScripts = [];
    ...
    this.bindTips = function( $context ){
      $('*[title]', $context).each(
        function(i, o){
            $(o).bind('mouseover.owtip',
                function(){
                    OW.showTip($(this), {hideEvent: 'mouseout'});
                }
           ).bind('mouseout.owtip',
                function(){
...
function OW_FloatBox(options)
{
    ...
    if (options.position)
    {
        if (options.position.left) this.$container.css('margin-left', options.position.left);
        if (options.position.top) this.$container.css('margin-top', options.position.top);
    }
...
OW_FloatBox.prototype = {
    ...
    /*fitWindow: function( params )
    {
        ...
        minMargin = this.options.layout == 'empty' ? 40 : 20;

        css.marginTop = ( jQuery(window).height() / 2 ) - ( (params.height || this.$container.height()) /2 + 100 );
        css.marginTop = css.marginTop < minMargin ? minMargin : css.marginTop;
...
OW_PingCommand.PROTO = function()
{
    this._received = function( r )
    {
        ...
        if ( this.stopped || this.repeatTime === false )
        {
            return;
        }
...
OW_Ping = function()
{
    ...
    var pingTimeoutId;
    var working = false;
    var _pingInterval = 1000, rspUrl;

    var sendStack, delayPing;
    ...
    this.addCommand = function( commandName, commandObject )
    {
        ...
        return _commands[commandName] = new OW_PingCommand(commandName, commandObject, _stack);
    };
    ...
    this.stop = function()
    {
        if ( pingTimeoutId )
        {
            window.clearInterval(pingTimeoutId);
        }
