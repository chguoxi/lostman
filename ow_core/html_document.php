...
class OW_HtmlDocument extends OW_Document
{
    ...
    {
        if ( $attributeName !== 'http-equiv' && $attributeName !== 'name' )
        {
    ...
    public function render()
    {
        ...
        {
            foreach ( $this->meta['name'] as $name => $content )
            {
                $attrs = array('name' => $name, 'content' => $content);
                $headData .= UTIL_HtmlTag::generateTag('meta', $attrs) . PHP_EOL;
            }
        }

        if ( !empty($this->meta['http-equiv']) )
        {
            foreach ( $this->meta['http-equiv'] as $name => $content )
            {
                $attrs = array('http-equiv' => $name, 'content' => $content);
                $headData .= UTIL_HtmlTag::generateTag('meta', $attrs) . PHP_EOL;
            }
        }
        ...
        foreach ( $this->preIncludeJavaScriptDeclarations as $priority => $types )
        {
            foreach ( $types as $type => $declarations )
            {
                foreach ( $declarations as $declaration )
                {
                    ...
                    $headData .= UTIL_HtmlTag::generateTag('script', $attrs, true, PHP_EOL . $declaration . PHP_EOL) . PHP_EOL;
                }
            }
        }
        ...
        foreach ( $this->javaScripts['items'] as $priority => $types )
        {
            foreach ( $types as $type => $urls )
            {
                foreach ( $urls as $url )
                {
                    ...
                    $headData .= UTIL_HtmlTag::generateTag('script', $attrs, true) . PHP_EOL;
                }
            }
        }
        ...
        foreach ( $this->styleDeclarations['items'] as $priority => $mediaTypes )
        {
            foreach ( $mediaTypes as $media => $declarations )
            {
                foreach ( $declarations as $declaration )
                {
                    ...
                    $headData .= UTIL_HtmlTag::generateTag('style', $attrs, true, $declaration);
                }
            }
        }
        ...
        foreach ( $this->javaScriptDeclarations['items'] as $priority => $types )
        {
            foreach ( $types as $type => $declarations )
            {
                ...
                {
                    $attrs = array('type' => $type);
                    $headData .= UTIL_HtmlTag::generateTag('script', $attrs, true, PHP_EOL . '(function() {' . $declaration . '})();' . PHP_EOL) . PHP_EOL;
                }
            }
        }

        // ONLOAD JS
        $headData .= '<script type="text/javascript">' . PHP_EOL . '$(function () {' . PHP_EOL;
        ...
        foreach ( $this->onloadJavaScript['items'] as $priority => $scripts )
        {
            ...
            {
                $headData .= '(function() {' . $script . '})();' . PHP_EOL;
            }
        }

        $headData .= PHP_EOL . '});' . PHP_EOL . '</script>';
        ...
        $headData .= implode('', $this->customHeadInfo);
        ...
        $assignArray = array(
            ...
            'pageBody' => $this->prependCode . $masterPageOutput . $this->appendCode. OW_Document::APPEND_PLACEHOLDER);
