...
//~~		
    ...
    public function mod()
    {
        ...
        {
            $za = new ZipArchive();
            // collect lang tags for archive name
            $exportedLangTags = array();
            ...
            foreach ( $_POST['set']['lang'] as $key => $value )
            {
                ...
                $exportedLangTags[] = $langDto->getTag();
            ...
            if ( file_exists($archivePath) )
            {
                ...
                header('Content-Disposition: attachment; filename=' . 'lang-dump-' . implode('-', $exportedLangTags) . '-' . date('d-m-y') . '.zip');
        ...
        $active_langs = !empty($active_langs) ? $active_langs : array();
        $inactive_langs = !empty($inactive_langs) ? $inactive_langs : array();
        ...
        $this->addForm(new ImportLangForm());
...
class CloneForm extends Form
{
    ...
    function __construct()
    {
        ...
        if ( !OW::getRequest()->isAjax() )
            OW::getDocument()->addOnloadScript("owForms['{$this->getName()}'].bind('success', function(){location.href = location.href;});");
    }
}