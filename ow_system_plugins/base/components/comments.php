    ...
    public function __construct( BASE_CommentsParams $params )
    {
        ...
        if ( OW::getUser()->isAuthorized($params->getPluginKey(), 'add_comment') && $params->getAddComment() )
        {
            ...
            else
            {
                ...
                $attachmentInputId = $form->getElement('attch')->getId();

                $inputId = $form->getElement('commentText')->getId();
                $js = 'OWLinkObserver.observeInput("' . $inputId . '", function(link){
                    var ac = $("#attachment_preview_' . $id . '");
                ...
                OW::getDocument()->addOnloadScript(
                    "window.owCommentCmps['$id'] = new OwComments('$cmpContextId', '$formName', '$id');
                    owForms['{$form->getName()}'].bind('success', function(data){
                        window.owCommentCmps['$id'].updateCommentsCountOnPage(0);
                        window.owCommentCmps['$id'].repaintCommentsList(data);
